...
class AudioVisualGenerator(Generator):
    ...
    def _get_samples(self, data_file:str, label_file:str):
        ...
        visual_frame = np.array(list(clip.iter_frames()))
        
        if self.detector:
            try:
                visual_frame_face = self.detector.extract_and_resize_face(visual_frame)
            except:
                visual_frame_face = np.zeros(
                    (1, self.detector.resize[0], self.detector.resize[0], 3), dtype=np.float32)
        visual_frame = visual_frame.transpose(0, 3, 1, 2).astype(np.float32)
        visual_frame_face=visual_frame_face.transpose(0,3,1,2).astype(np.float32)
        
        audio_frame = np.array(list(clip.audio.iter_frames()))
        audio_frame = audio_frame.mean(1).astype(np.float32)

        labels = np.array(labels).astype(np.float32)
        
        num_samples = [audio_frame.shape[0], visual_frame_face.shape[0], visual_frame.shape[0]]
        data = [audio_frame,visual_frame_face,visual_frame]
        return data, labels, 1, num_samples, attrs_name
    ...
    def serialize_samples(self, writer:h5py.File, data_file:str, label_file:str):
        ...
        """
        file_data, attrs_name = self.labelfile_reader.read_file(label_file)
        tmp=np.array(file_data)
        print(file_data)
        if int(tmp[0][4])>3:
            return
        frames, labels, seq_num, num_samples, names = self._get_samples(data_file, label_file)
        audio_frames, visual_frames_face, visual_frames = frames
        
        audio_num_samples, visual_num_samples_face, visual_num_samples = num_samples
        
        # store data
        
        writer.create_dataset('audio', data=audio_frames,compression="lzf")
        writer.create_dataset('visual_face',data=visual_frames_face,compression="lzf")
        writer.create_dataset('visual', data=visual_frames,compression="lzf")
        writer.create_dataset('labels', data=labels,compression="lzf")
        
        # Save meta-data
        writer.attrs['data_file'] = str(data_file)
        writer.attrs['label_file'] = str(label_file)
        writer.attrs['seq_num'] = seq_num
        writer.attrs['num_samples'] = [audio_num_samples, visual_num_samples_face, visual_num_samples]
