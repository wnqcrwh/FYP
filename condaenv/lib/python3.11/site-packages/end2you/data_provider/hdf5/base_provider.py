...
class BaseProvider(Dataset):
    ...
    def __init__(self,
        ...
        # self.data_files = natsorted(list(Path(dataset_path).glob('**/*.hdf5')))
        pattern=re.compile(r'dia[0-3]_utt\d+\.hdf5$')
        self.data_files = natsorted([file for file in Path(dataset_path).glob('**/*.hdf5') if pattern.search(file.name)])
